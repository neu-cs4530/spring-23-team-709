...
/**
 * A ListeningAreaController manages the state for a ListeningArea in the frontend app, serving as a bridge between the song
 * that is playing in the user's browser and the backend TownService, ensuring that all players watching the same song
 * are synchronized in their playback.
 *
 * The ListeningAreaController implements callbacks that handle events from the song player in this browser window, and
...
export default class ListeningAreaController extends (EventEmitter as new () => TypedEventEmitter<ListeningAreaEvents>) {
  ...
  /**
   * The URL of the song assigned to this listening area, or undefined if there is not one.
   */
  public get song() {
